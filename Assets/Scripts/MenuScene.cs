...
public class MenuScene : MonoBehaviour {
	...
	public GameObject AGVBar;
	public GameObject FFBar;
	public GameObject MagnetBar;
	public GameObject FreqBar;
	public Sprite AGVLockedImage;
	public Sprite FFLockedImage;
	public Sprite MagnetLockedImage;
	public Sprite FreqLockedImage;
	public Sprite AGVImage;
	public Sprite FFImage;
	public Sprite MagnetImage;
	public Sprite FreqImage;
	...
	void Start () {
		GM.FirstRun = true;
		Debug.Log ("it is main menu");
		//saveManager = gameManager.GetComponent<SaveManager> ();
		volumeManager = settingsMenu.GetComponent<VolumeManager>();
		...
		tutorialCleared = !SaveManager.Instance.FirstTime ();
...
//		helpCanvas.blocksRaycasts = false;
		...
		isPowerUpLocked(AGVBar, SaveManager.Instance.getAGV());
		isPowerUpLocked(FFBar, SaveManager.Instance.getBoat());
		isPowerUpLocked(MagnetBar, SaveManager.Instance.getMagnet());
		isPowerUpLocked(FreqBar, SaveManager.Instance.getPowerups());
	...
	void Update () {
		...
		//Fade - in
		timeFromStart+=(currentTime-previousTime);
		fadeGroup.alpha = 1 - timeFromStart * fadeInSpeed;
	...
	public void onHelpClick(){
		//SaveManager.Instance.tutorialReset ();
		clickSound.Play ();
...
//	}
	...
	private void isPowerUpLocked(GameObject bar, int level){
		if (level == 1){
			if (bar == AGVBar) {
				bar.GetComponentInChildren<Image> ().sprite = AGVLockedImage;
				AGVLevel.SetActive(false);
				

			}
			if (bar == MagnetBar) {
				bar.GetComponentInChildren<Image> ().sprite = MagnetLockedImage;
				MagnetLevel.SetActive(false);
			}
			if (bar == FFBar) {
				bar.GetComponentInChildren<Image> ().sprite = FFLockedImage;
				BoatLevel.SetActive(false);

			}
			if (bar == FreqBar) {
				bar.GetComponentInChildren<Image> ().sprite = FreqLockedImage;
				PowerupLevel.SetActive(false);

			}
		} else {
			if (bar == AGVBar) {
				bar.GetComponentInChildren<Image> ().sprite = AGVImage;
				AGVLevel.SetActive(true);
			}
			if (bar == MagnetBar) {
				bar.GetComponentInChildren<Image> ().sprite = MagnetImage;
				MagnetLevel.SetActive(true);
			}
			if (bar == FFBar) {
				bar.GetComponentInChildren<Image> ().sprite = FFImage;
				BoatLevel.SetActive(true);
			}
			if (bar == FreqBar) {
				bar.GetComponentInChildren<Image> ().sprite = FreqImage;
				PowerupLevel.SetActive(true);
			}
		}
	}
	...
	private void InitButton(GameObject button, int level){ //to make sure the button displays the correct price based on level
		if (button == LivesBuyButton) {
			...
			case 3:
				...
				break;
			case 4:
				button.GetComponentInChildren<Text> ().text = "200000";
				break;
			case 5:
				button.GetComponentInChildren<Text> ().text = "500000";
				break;
			case 6:
